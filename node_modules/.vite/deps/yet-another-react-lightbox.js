...
function ImageSlide({ slide: image, offset, render, rect, imageFit, onClick, onLoad, style }) {
  ...
  return React.createElement(
    React.Fragment,
    null,
    React.createElement("img", { ref: setImageRef, onLoad: handleOnLoad, onError, onClick, className: clsx(cssClass(slideImagePrefix()), cover && cssClass(slideImagePrefix("cover")), status !== SLIDE_STATUS_COMPLETE && cssClass(slideImagePrefix("loading"))), draggable: false, alt: image.alt, style: { ...defaultStyle, ...style }, sizes, srcSet, src: image.src }),
...
function CarouselSlide({ slide, offset }) {
  ...
  const renderSlide = () => {
    ...
    if (!rendered && isImageSlide(slide)) {
      rendered = React.createElement(ImageSlide, { slide, offset, render, rect: slideRect, imageFit, onClick: offset === 0 ? () => onClick === null || onClick === void 0 ? void 0 : onClick({ index: currentIndex }) : void 0 });
    }
...
function NavigationButton({ label: label2, icon, renderIcon, action, onClick, disabled }) {
  return React.createElement(IconButton, { label: label2, icon, renderIcon, className: cssClass(`navigation_${action}`), disabled, onClick, ...useLoseFocus(disabled) });
}
